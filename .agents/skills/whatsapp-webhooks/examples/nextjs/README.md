# WhatsApp Webhooks - Next.js Example

Minimal example of receiving WhatsApp (Meta Cloud API) webhooks in a Next.js App
...
The route handler lives at `app/webhooks/whatsapp/route.ts`:
...
- **Raw body** — `await request.text()` is read before parsing, so the signature is
...
- **Echo the challenge** — the GET handshake returns the raw `hub.challenge` as plain
  text (`new NextResponse(challenge)`), not JSON.
